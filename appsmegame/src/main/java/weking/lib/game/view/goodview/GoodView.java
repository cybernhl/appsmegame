...
public class GoodView extends PopupWindow implements IGoodView {
    ...
    public GoodView(Context context, int textColor) {
        super(context);
        mContext = context;
        if (textColor == 0) {
            initView(mTextColor);
            ...
            // 获取颜色
            initView(ContextCompat.getColor(mContext, textColor));
        }
    }
    ...
    private void initView(int textColor) {
        ...
        mGood = new StrokeTextView(mContext, Color.BLACK, textColor);
...
//        mGood.setTextColor(textColor);
    ...
    public void setText(String text, Typeface mTypeFace) {
        if (TextUtils.isEmpty(text)) {
            throw new IllegalArgumentException("text cannot be null.");
        }
        if (mTypeFace != null) {
            mGood.setTypeface(mTypeFace);
        }
        mText = text;
        mGood.setText(text);
    ...
    public void show(View v) {
        Log.d("mAnimationSet ", "show:   " + v.toString());
        if (!isShowing()) {
            showAsDropDown(v, v.getWidth() + 3, -getHeight());
    ...
    public void showTop(View v) {
        Log.d("mAnimationSet ", "show:   " + v.toString());
        if (!isShowing()) {
//            showAsDropDown(v, v.getWidth() / GameAnimUtil.getTextWidth(mGood) / 2, -getHeight());
            showAsDropDown(v, v.getWidth() / 2 - GameAnimUtil.getTextWidth(mGood), v.getHeight()/2);
            if (mAnimationSet == null || mChanged) {
                mAnimationSet = createAnimation();
                mChanged = false;
            }
